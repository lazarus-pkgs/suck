...
extern const char *default_queue_phrases[];
...
int main(int argc, char *argv[]) {
	...
	if(argc != 2) {
		...
		if(retval == 0) {
			retval = do_one_array(fpo, nr_queue_phrases, default_queue_phrases);
		}
...
int do_phrases_h(void) {
	...
	if((fpo = fopen(PHRASES_H, "w")) == NULL) {
		...
		i = count_vars(default_queue_phrases, nr_queue_phrases);
		if(i > maxvar) {
			maxvar = i;
		}
		...
		fprintf(fpo, "#define NR_QUEUE_PHRASES %d\n\n", nr_queue_phrases);
		...
		fprintf(fpo, "extern char *default_queue_phrases[];\n");
		...
		fprintf(fpo, "extern char **queue_phrases;\n");
